...
```
## Introduction
In this course we have learned how to manipulate, tidy, and query data 
via matrices, vectors, lists, and data frames. With vectors we can subscript with bracket notation to query data, alike, by using the  "dplyr" package, we can query and manipulate data frames. R is a powerful language and like many other languages gains its power from the extension of libraries. For one, packages simplify calculations with elegance and extent the flexibility of our programs while abstracting junky and lengthy calculations. And they allows us to personalize our working environment, one that we may be comfortable with and can be more efficient with.

Amongst the myriad of languages, a developer has a tough choice in selecting a tool that best fits their need. While one language may offer more flexibility another might provide efficiency. A developer first query languages that fit their need but how do they then proceed and make a selection? Although is it key that a developer is flexible and can adapt to different working environments it is often the case that developers find comfort in working environments and tools that are more native and natural to them. One language might have a slight edge over another but this doesn't mean the developer should tend to the "better" one. Often, the developer tends to the one they are more comfortable with and the one that has the necessary and sufficient power to carry out their tasks.

Some packages give developers the best of both worlds; they allow a developer to use a language that they are comfortable with while using the utility of another.This post is about extending R to use SQL commands via the ["sqldf" package](https://github.com/ggrothendieck/sqldf#example-4-join) to manipulate and query data frames. Although this post will draw many parallels to the "dplyr" package, I will not be comparing the two but rather informing of an additional tool that you might be more comfortable with and prefer. So let's get started!
...
At its core, a SQL command is structured as: SELECT columns WHERE columns meet the imposed conditions GROUPed by a column  HAVING an imposed limitation ORDERed by a column. Here, you first filter your columns, you then filter the rows, you then aggregate the data, and follow by limiting the data and ordering the results
...
Back to our example, we know that we have two tables in our database, "clothing" and "shoes", and we know their columns.  In our working days, before the store opens, we want to check our inventory for both clothing and shoes.
...
Where the first column is price followed by size, brand, category (for clothing only), and gender.
Now suppose we only wanted certain columns, say we only want to view the available brands. We can replace "*" and specify a column name, for example 
```{r eval=FALSE}
...
Suppose now we can a bit of a more complicated query. Our example database is relatively small, in the case that we ran a huge thrift store the queries mentioned earlier might return thousands upon thousands of columns. We might want to be more specific in our queries. Supposed we are both fanatics of the "Nike" brand and we often find mint condition shoes in our inventory. So now we want to query our database to return only the Nike branded shoes. From our previous SQL command structure: select column from table, we will add select column from table WHERE CONDITION. That is, WHERE is a clause (like Select) and CONDITION is a logical condition on the the columns that filter the returned rows. From the previous example "select * from shoes"
our new query will 
```{r eval=FALSE}
SELECT * from shoes WHERE brand == 'Nike';
```
Here our condition is "brand == 'Nike". The following rows are returned:


![](/Users/Roberto/Desktop/out4.png)


However, let's assume that this query still returns to many rows and we specifically just want to view Nike shoes that are equal to or more than $20. We add the condition "price => 20" to our query and we get:
...
Similarly we could have grouped our columns any other column like "size" or "gender". (Note: the GROUP BY clause is very similar to group_by() from the "dplyr" package). 
...
Now suppose are tables were filled accordingly and we ran the following query:
